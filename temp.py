...
class INS_GUI:
    ...
    def create_widgets(self):
        ...
        tk.Label(header,
                 text="INS/GNSS  Loosely & Tightly Coupled Algorithms",
        ...
        # Collect LCA_PreRec*_config.txt and TCA_PreRec*_config.txt.
        # Group by PreRecN; store both algo paths per dataset.
        import re
        _pat = re.compile(
            r"(LCA|TCA)_PreRec(\d+)_config\.txt$", re.IGNORECASE)
        _dataset_map = {}   # prerec -> {'LCA': path|None, 'TCA': path|None}
        for p in sorted(
            glob.glob(os.path.join(script_dir, "LCA_PreRec*_config.txt")) +
        ...
        if _dataset_map:
            for prerec_num in sorted(_dataset_map):
                ...
                cb = tk.Checkbutton(
                    row_frame,
                    text=f"Prerecorded Dataset {prerec_num}",
                    variable=var,
            ...
            tk.Label(ds_card,
                     text="⚠  No LCA/TCA_PreRec*_config.txt files found in script directory.",
    ...
    def ask_outage_changes(self, config_file, default_outages,
        ...
        if include_sat_count:
            info_text = (
            "Edit start / end times and max satellite count during outage below.\n"
                ...
                try:
                    ...
                    if include_sat_count:
                        ...
                        if sc not in [0, 3]:
                            messagebox.showerror(
                                "Invalid Input",
                                f"Row {i+1}: max sat count must be 0 or 3.",
                                parent=dialog)
                            return
                except ValueError:
                    messagebox.showerror(
                        "Invalid Input",
                        f"Row {i+1}: start, end, and max sat count must be numbers.",
                        parent=dialog)
                    return
    ...
    def compute_statistics_with_outages(self, outages_to_use):
        ...
        return (non_outage,
                (hrms, hmax, vrms, vmax_mag, avg_sat_count),
                (eohrms, eohmax, eovrms, eovmax_mag, avg_sat_count_outage))
    ...
    def show_plots(self):
        ...
        if self.traj_df is not None and self.gnss_ref_df is not None and not self.traj_df.empty and not self.gnss_ref_df.empty:
            ...
            traj_line = ax_map.plot(self.traj_df['lon_deg'], self.traj_df['lat_deg'],
                       color='#1f77b4', linewidth=1.5, alpha=0.8, 
                       label='INS/GNSS Estimated', zorder=1)[0]
            
            # Plot ALL GNSS reference points as small circles (not a line)
            gnss_scatter = ax_map.scatter(self.gnss_ref_df['lon_deg'],
            ...
            for start, end in outages_to_use_map:
                ...
                if not outage_traj.empty:
                    ...
                    line = ax_map.plot(outage_traj['lon_deg'], outage_traj['lat_deg'],
                               color='red', linewidth=2.5, alpha=0.9, zorder=3)[0]
            ...
            # Store references and canvas for visibility toggling
            canvas_ref = [None]
            visibility_state = {
                'INS/GNSS Estimated': [True, traj_line],
                'Outage Periods': [True, outage_lines],
                'GNSS Reference': [True, gnss_scatter],
                'Start / End Points': [True, [start_pt, end_pt]]
            }
            
            def create_toggle_callback(key):
                def toggle_visibility():
                    state = visibility_state[key]
                    state[0] = not state[0]
                    
                    if key == 'INS/GNSS Estimated':
                        if state[1]:
                            state[1].set_visible(state[0])
                    elif key == 'Outage Periods':
                        for line in state[1]:
                            line.set_visible(state[0])
                    elif key == 'GNSS Reference':
                        if state[1]:
                            state[1].set_visible(state[0])
                    elif key == 'Start / End Points':
                        for pt in state[1]:
                            pt.set_visible(state[0])
                    
                    if canvas_ref[0] is not None:
                        canvas_ref[0].draw()
                return toggle_visibility
            
            # Create the tab with Tkinter checkbuttons
            tab_frame = tk.Frame(notebook, bg=C_BG)
            notebook.add(tab_frame, text="Trajectory map")
            
            # Canvas frame (takes up most of the space)
            canvas_frame = tk.Frame(tab_frame, bg=C_BG)
            canvas_frame.pack(fill=tk.BOTH, expand=True, side=tk.TOP)
            
            toolbar_frame = tk.Frame(canvas_frame, bg=C_BG)
            toolbar_frame.pack(side=tk.TOP, fill=tk.X)
            map_canvas = FigureCanvasTkAgg(fig_map, master=canvas_frame)
            ...
            # Control panel at the bottom left
            control_frame = tk.Frame(tab_frame, bg="#e0e0e0", relief=tk.RAISED, bd=1)
            control_frame.pack(side=tk.BOTTOM, fill=tk.X, padx=4, pady=4, anchor=tk.W)
            
            label = tk.Label(control_frame, text="Toggle Layers:", bg="#e0e0e0", fg=C_TEXT, font=FONT_LABEL)
            label.pack(side=tk.LEFT, padx=8, pady=6)
            
            button_vars = {}
            for key in ['INS/GNSS Estimated', 'Outage Periods', 'GNSS Reference', 'Start / End Points']:
                var = tk.BooleanVar()
                button_vars[key] = var
                btn = tk.Checkbutton(
                    control_frame, text=key, variable=var,
                    command=create_toggle_callback(key),
                    bg="#e0e0e0", fg=C_TEXT, font=FONT_LABEL,
                    activebackground=C_BADGE_BG, activeforeground=C_TEXT
                )
                btn.pack(side=tk.LEFT, padx=4, pady=6)
                var.set(True)  # Set the variable to True after packing
                btn.update()   # Force visual update
